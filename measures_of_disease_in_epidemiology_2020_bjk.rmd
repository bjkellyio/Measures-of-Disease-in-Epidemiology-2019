...
# Measures of Disease Occurrence:<br/>Prevalence
...
- To understand the difference between point and period prevalence, let's imagine a cohort of people living with HIV/AIDS at risk for _Pneumocystis_:  
    ...
    - cases of PCP each receive 3 weeks of antibiotic treatment (red dots and lines on plot)  
...
## Cohort at Risk for _Pneumocystis_ <hr/>
...
## Cohort at Risk for _Pneumocystis_: Point Prevalence <hr/>
...
## Cohort at Risk for _Pneumocystis_: Period Prevalence <hr/>
...
# Measures of Disease Occurrence:<br/>Incidence
...
$$cumulative\ incidence = \frac{{new\ cases}}{population\ at\ risk\\at\ start\ time\ interval}$$  
...
## Cohort at Risk for _Pneumocystis_: Cumulative Incidence (at risk at start) <hr/>
...
## Cohort at Risk for _Pneumocystis_: Incidence Density (person-time at risk) <hr/>
...
# Measures of Disease Occurrence:<br/>Prevalence vs Incidence
...
- But do NOT know this generally in case control studies: pick the cases and control groups, then measure rates of exposure (do NOT know how many in population at risk).  
...
# 2x2 Table: Calculate Relative Risk for Cohort/Trial <hr/>
...
# _Pneumocystis_ ~ CD4 Cell Count: Relative Risk (RR) <hr/>

.pull-left[

- Imagine a cohort study examining incidence (risk) for _Pneumocystis_ among patients with low vs high (dichotomous) CD4 cell counts:
...
tibble(`Low CD4` = factor(x = c("+","-"), levels = c("-","+")), Pneumocystis = factor(x = c("+","-"), levels = c("+","-"))) %>%
  complete(`Low CD4`,Pneumocystis) %>%
  mutate(cells = c("5","15","10","10"),
         cells_order = factor(x = c(3,4,1,2))) %>%
  ggplot(data = .) +
  geom_tile(aes(x = Pneumocystis, y = `Low CD4`, fill = cells_order)) +
  geom_text(aes(x = Pneumocystis, y = `Low CD4`, label = cells), size = 16) +
  scale_x_discrete(position = 'top') +
  ggsci::scale_fill_nejm() +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 28),
...
# 2x2 Table: Calculate NNT for Cohort/Trial <hr/>
...
# _Pneumocystis_ ~ CD4 Cell Count: Number Needed to Treat (NNT) <hr/>

.pull-left[

- Cohort study examining incidence (risk) for _Pneumocystis_ among patients with low vs high (dichotomous) CD4 cell counts:
...
## Absolute Risk (AR), Risk Difference (RD) & Number Needed to Treat (NNT) <hr/>
...
- **risk difference (RD)**: difference in risk between the treatment/exposure group and the control group
    ...
    - if 3 in 10 are cured in the treatment group and 2 in 10 in the control group, the RD is 3/10 – 2/10 = 10%

- **number needed to treat (NNT)**: number of patients who need to be treated for one person to benefit from the treatment  (= 1/RD)

    - using RD numbers above, NNT = 1/RD = 1/0.1 = 10 (treat 10 people to cure one additional person)


---

# 2x2 Table: Calculate Odds Ratio for Case-Control Study <hr/>
...
# _Pneumocystis_ Cases vs Controls: Odds Ratio (OR) <hr/>

.pull-left[

- Imagine a different study: 100 _Pneumocystis_ cases and 100 controls; measure whether 'low CD4' (exposure) differs between groups chosen by investigator.
...
# Utility of Odds Ratio (OR) & Case-Control Study Design <hr/>
...
# ... how did we get dichotomous exposures and outcomes?
