...
- Remember the diagnosis lecture: no perfect threshold of oxygen saturation with which to diagnose _Pneumocystis_ pneumonia.
...
- To understand the difference between cumulative incidence and incidence density, imagine a study of persons with HIV at risk for PCP:  
    ...
    - 100 high-risk subjects enrolled at the start of a two-year observation period  
...
```{r pcp_prevalence, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4.5, fig.width=12, fig.align='center', fig.retina=2}

library(tidyverse)

tibble(subject_id = factor(seq(100)),
       enrollment = rep(0,100),
       exit = sample(x = c(rep(2,70),runif(n = 30, min = 1.7, max = 2)), size = 100, replace = FALSE),
       pcp_start = c(rep(NA,10),1.2,rep(NA,10),0.4,rep(NA,8),1.1,rep(NA,12),0.8,rep(NA,9),1.4,NA,0.3,rep(NA,8),1.0,rep(NA,18),0.1,rep(NA,11),0.9,rep(NA,4)),
       pcp_stop = pcp_start + 0.0577,
       ppx_start = c(rep(NA,8),1.5,rep(NA,12),1.6,rep(NA,13),1.4,rep(NA,7),1.7,rep(NA,6),1.4,rep(NA,5),1.3,rep(NA,4),1.55,rep(NA,21),1.2,rep(NA,10),0.9,rep(NA,5))) %>%
  group_by(subject_id) %>%
  nest() %>%
  mutate(exit_adjust = map_dbl(data, ~ ifelse(is.na(.x[["pcp_start"]]) & is.na(.x[["ppx_start"]]), sample(x = seq(1.3,2,0.1), size = 1), .x[["exit"]]))) %>%
  unnest(cols = c(data)) %>%
  ungroup() %>%
  mutate(ppx_stop = exit_adjust) %>%
  ggplot(data = .) +
  geom_segment(aes(x = enrollment, xend = exit_adjust, y = subject_id, yend = subject_id), colour = "black", size = 1) +
  ...
  theme_classic() +
  labs(x = "Duration of Study Enrollment (years)", y = "Subject ID") -> p

p + geom_vline(aes(xintercept = 0.9), colour = "green", linetype = 21)
