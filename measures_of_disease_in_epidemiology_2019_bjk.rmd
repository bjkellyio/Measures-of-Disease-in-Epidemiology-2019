...
- New data would show an increasing <ins>incidence</ins>. (In ID, the differential is always evolving).  
...
<p style="text-align: center;"> (you do <strong>NOT</strong> need to memorize these formulas) </p>
...
- “Non-parametric” data have a mean and standard deviation, but these parameters do NOT characterize the data well:  
    ...
    - **skewed** data: distrubtion not symmetric around a central value  
...
tibble("Uniform" = runif(n = 1000, min = 0.82, max = 0.94), "Skewed" = rbeta(1000,9,1)) %>%
...
- Median and mode depend on ranking the values:
    ...
    - median is the “middle value” when data are ordered / ranked 
...
- Remember the diagnosis lecture: threshold of oxygen saturation (or \beta-D glucan) to diagnose _Pneumocystis_ pneumonia means trade-off between sensitivity and specificity.
...
## Point Prevalence versus Period Prevalence <hr/>
...
- To understand the difference between point and period prevalence, let's imagine a cohort of people living with HIV/AIDS at risk for _Pneumocystis_:  
    ...
    - cases of PCP each receive 3 weeks of antibiotic treatment (red dots and lines on plots)  
    
    - incomplete follow-up (abbreviated grey lines on plot)
    
    - towards end of study, some subjects started on _Pneumocystis_ prophylaxis (blue lines on plot)  


---

## Cohort of Patients at Risk for _Pneumocystis_ <hr/>

```{r pcp_study, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=12, fig.align='center', fig.retina=2}
...
```{r print_pcp_study, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=12, fig.align='center', fig.retina=2, dependson= "pcp_study"}
...
## Cohort of Patients at Risk for _Pneumocystis_: Point Prevalence <hr/>

```{r pcp_point_prev, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=12, fig.align='center', fig.retina=2, dependson= "pcp_study"}

p + geom_vline(aes(xintercept = 0.9), colour = "green", size = 3, alpha = 0.6)
...
## Cohort of Patients at Risk for _Pneumocystis_: Period Prevalence <hr/>

```{r pcp_period_prev, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=12, fig.align='center', fig.retina=2, dependson= "pcp_study"}

p + annotate("rect", xmin=0.9, xmax=1.2, ymin=0, ymax=Inf, alpha=0.6, fill="green")
...
.footnote[[1,2] Masur H et al _NEJM_ 1981; Gottlieb MS et al _MMWR_ 1981]
...
```{r reprint_pcp_study, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=12, fig.align='center', fig.retina=2, dependson= "pcp_study"}
...
## Cohort of Patients at Risk for _Pneumocystis_: Cumulative Incidence <hr/>

```{r pcp_incid_proportion, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=12, fig.align='center', fig.retina=2, dependson= "pcp_study"}

p + geom_point(aes(x = 0, y = subject_id), colour = "green", size = 3, alpha = 0.6) +
  geom_segment(aes(x = enrollment, xend = 2, y = subject_id, yend = subject_id), colour = "green", size = 2, alpha = 0.6, linetype = 21)
...
## Cohort of Patients at Risk for _Pneumocystis_: Incidence Density <hr/>

```{r pcp_incid_density, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=12, fig.align='center', fig.retina=2, dependson= "pcp_study"}

p + geom_point(aes(x = 0, y = subject_id), colour = "green", size = 3, alpha = 0.6) +
  geom_segment(aes(x = follow_start1, xend = follow_stop1, y = subject_id, yend = subject_id), colour = "green", size = 2, alpha = 0.6, linetype = 21) +
    geom_segment(aes(x = follow_start2, xend = follow_stop2, y = subject_id, yend = subject_id), colour = "green", size = 2, alpha = 0.6, linetype = 21)
...
## Beware the Phrase “Incidence Rate”! <hr/>
...
.pull-left[
- <mark>Dichotomous</mark> exposures and outcomes.
...
## Absolute Risk (AR) & Reduction (RD) <hr/>
...
- risk difference (RD): difference in risk between the treatment/ exposure group and the control group

    - if 2 in 10 are cured in the control group and 3 in 10 in the treatment group, the RD is 3/10 – 2/10 = 10%

---

## Number Needed to Treat (NNT) <hr/>

- Number needed to treat (NNT): Number of patients who need to be treated for one person to benefit from the treatment  (= 1/RD)

    - using RD numbers above, NNT = 1/RD = 1/0.1 = 10
...
- risk difference (RD) = not appropriate to calculate
