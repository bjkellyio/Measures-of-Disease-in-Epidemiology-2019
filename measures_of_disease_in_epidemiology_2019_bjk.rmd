...
- To understand the difference between point and period prevalence, let's imagine a cohort of people living with HIV/AIDS at risk for _Pneumocystis_:  
    ...
    - at end of study, subjects started on _Pneumocystis_ prophylaxis (blue lines on plot)  
...
tibble(subject_id = factor(seq(50)),
  ...
  group_by(subject_id) %>%
  mutate(follow_stop = min(ppx_start, exit_adjust, na.rm = TRUE)) %>%
  ungroup() %>%
  ggplot(data = .) +
...
p + geom_point(aes(x = 0, y = subject_id), colour = "green", size = 3, alpha = 0.6) +
  geom_segment(aes(x = enrollment, xend = follow_stop, y = subject_id, yend = subject_id), colour = "green", size = 2, alpha = 0.6, linetype = 21)
