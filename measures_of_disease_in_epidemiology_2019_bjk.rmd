...
pcp_o2 %>%
  ggplot(data = .) + 
  geom_histogram(aes(x = `O2 Saturation`, y = ..density.., fill = sd_type)) +
  #geom_density(aes(x = `O2 Saturation`), colour = "black", fill = NA, kernel = 'gaussian', weight = 2) +
  geom_line(data = pcp_norm, aes(x = `O2 Saturation`, y = norm_density), colour = "black") +
  ...
  labs(title = "1000 Subjects with Pneumocystis*", x = "O2 Saturation (%)", y = "Density")
...
- In 1981, _Pneumocystis_ was known to be a low **prevalence** disease.

- But new data would show an increasing **incidence** in certain populations.
<br/>
<br/>
<br/>
...
# Measuring True Risk versus Odds <hr/>

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.align='left', fig.width=16, fig.height=5, fig.retina=2}
library(DiagrammeR)
...
c_graph <-
  ...
  add_node(label = 'Denominator\nUnknown', node_aes = node_aes(fontsize = 6, color = 'black', fillcolor = 'dodgerblue', fontcolor = 'black')) %>%
  ...
  add_edge(from = 'Case-\nControl', to = 'Denominator\nUnknown', edge_aes = edge_aes(color = 'black')) %>%
...
- __Caution!!!__
    ...
    - need to know whether study is __RCT/cohort__ vs __case-control__<br/><br/>
    - can always calculate a __relative risk (RR)__ from 2x2 table but only appropriate for __RCT/cohort__<br/><br/>
    - can always calculate an __odds ratio (OR)__ from 2x2 table but only appropriate for __case-control study__<br/>(can do better with RR if RCT/cohort)
...
tibble(Exposure = factor(x = c("+","-"), levels = c("-","+")), Outcome = factor(x = c("+","-"), levels = c("+","-"))) %>%
  ...
  ggplot(data = .) +
  geom_tile(aes(x = Outcome, y = Exposure, fill = cells)) +
